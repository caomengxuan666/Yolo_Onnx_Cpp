...
void YOLODetector::initGPU(bool isGPU) {
    if (!isGPU) return;
...
void YOLODetector::loadModel(const std::string &modelPath) {
#ifdef _WIN32
    ...
    this->inputImageShape = cv::Size2f(cv::Size(640, 640));
}
...
YOLODetector::YOLODetector(bool isGPU) {
    std::cout<<"Intializing YOLO detector by    "<<__FUNCTION__ << std::endl;
...
#ifdef _WIN32
    std::string modelPath = "../models/yolov5m.onnx";
    std::wstring w_modelPath = utils::charToWstring(modelPath.c_str());
    session = Ort::Session(env, w_modelPath.c_str(), sessionOptions);
#else
    session = Ort::Session(env, modelPath.c_str(), sessionOptions);
